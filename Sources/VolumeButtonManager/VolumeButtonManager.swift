...
public class VolumeButtonManager {
    ...
    private var volumeView = MPVolumeView(frame: CGRect(x: CGFloat(MAXFLOAT), y: CGFloat(MAXFLOAT), width: 0, height: 0))
    ...
    private func observe() {
        ...
        /// similarly, if it's at 0.0, it's adjusted to 0.00001. This ensures that subsequentvolume changes, even when
        ...
        volumeObserver = audioSession.observe(\.outputVolume, options: [
            ...
            self.logger.log(status: .info, "Old Volume: \(oldValue), New Volume: \(newValue), Difference = \(difference)")

            if (newValue == 1.0 && oldValue == 1.0) || (newValue == 0.0 && oldValue == 0.0) {
                self.onAction?()
                self.setupInitialVolume()
                return
            } else {
                self.onAction?()
                self.isAdjustingInitialVolume = true
                self.setVolume(self.initialVolume)
                return
            }
        }
